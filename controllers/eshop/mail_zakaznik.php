...
for($num = 0; $num < count($itemsAll); $num++){
    if($itemsAll[$num] != "" && $countAll[$num] != "" && $countAll[$num] != "0" ) {
        ...
        $items[$number]['cenaZaKusBezDPH'] = number_format($thisItem['cenaBezDPH' . $languageForDb], 2, '.', '');
        $items[$number]['cenaZaKusSDPH'] = number_format($thisItem['cenaSDPH' . $languageForDb], 2, '.', '');
        $items[$number]['cenaBezDPH'] = number_format($countAll[$num]*$thisItem['cenaBezDPH' . $languageForDb], 2, '.', '');
        $items[$number]['cenaSDPH'] = number_format($countAll[$num]*$thisItem['cenaSDPH' . $languageForDb], 2, '.', '');
...
$cena = $doprava['cenaZaGram' . $languageForDb]*$zbytek + $doprava['cenaZaGram' . $languageForDb]*$ostatni + ($ostatni + 1)*$doprava['cenaZaJdenBalik' . $languageForDb];
$cena = (int) $cena;
